...
export default function ChatRoomBlock() {
  ...
  return (
    <div className="bg-zinc-100">
      <div className="divide-y divide-solid divide-lime-300">
        <h1 className="text-lg font-bold text-zinc-600  divide-y divide-dashed">Chatrooms</h1>
        <div className="mb-4"></div>
      </div>
      {profile.familyCode?.map((item) => (
        <Link className="flex items-start text-lime-600" key={item._id} to={item.link}>
      ...
      <Link
        to="/create-group"
        className="flex float-right mt-4 text-lime-500">
