...
function Message({ message, isOwnMessage }) {
  ...
  return (
    <div className="flex flex-col space-y-4 p-3 overflow-y-auto scrollbar-thumb-indigo scrollbar-thumb-rounded scrollbar-track-indigo-lighter scrollbar-w-2 scrolling-touch">
      ...
      <div className={classNames( isOwnMessage ? "flex flex-col space-y-2 text-sm max-w-xs mx-2 order-1 items-end " : "flex flex-col space-y-2 text-sm max-w-xs mx-2 order-2 items-start")}>
        <div
          className={classNames(
            isOwnMessage
              ? "px-4 py-2 rounded-lg inline-block rounded-br-none bg-indigo-600 text-white shadow-md "
              : "px-4 py-2 rounded-lg inline-block rounded-bl-none bg-gray-200 text-gray-600",
            "w-fit rounded-md shadow-md"
          )}
        >
          <h4 className="text-indigo-500 font-bold">{isOwnMessage ? "" : displayName}</h4>
          <div>{text}</div>
        </div>
      </div>
    </div>
